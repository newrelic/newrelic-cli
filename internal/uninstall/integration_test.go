...
// recipe fetcher, host filter, and Tier 1/Tier 2 executors, using the YAML
...
func TestIntegration_MetaFixtureFailsWithoutARealPackageManager(t *testing.T) {
	u := newFixtureUninstaller(true)

	res := u.Run(context.Background(), Options{RecipeName: "test-uninstall-meta", AssumeYes: true})
