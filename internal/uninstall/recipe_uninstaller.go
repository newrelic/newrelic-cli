...
const (
	...
	// StatusNoAutomatedUninstall means the recipe defines neither Uninstall nor UninstallMeta.
...
// TaskfileExecutor runs an arbitrary go-task Taskfile for a recipe (Tier 1).
...
// GenericExecutor performs declarative packages/services/paths removal (Tier 2).
type GenericExecutor interface {
	Uninstall(meta types.OpenInstallationUninstallMeta) (warnings []error, fatal error)
}
...
func (u *RecipeUninstaller) Run(ctx context.Context, opts Options) Result {
	...
	switch SelectTier(*recipe) {
	case TierTaskfile:
		if err := u.TaskfileExec.ExecuteTaskfile(ctx, recipe.Uninstall, *recipe, types.RecipeVars{}); err != nil {
			return Result{Status: StatusFailed, Err: err}
		}
		return Result{Status: StatusUninstalled}

	case TierGeneric:
		warnings, fatal := u.Generic.Uninstall(recipe.UninstallMeta)
		if fatal != nil {
			return Result{Status: StatusFailed, Warnings: warnings, Err: fatal}
		}
		if len(warnings) > 0 {
			return Result{Status: StatusFailed, Warnings: warnings, Err: fmt.Errorf("uninstall completed with %d warning(s)", len(warnings))}
		}
		return Result{Status: StatusUninstalled}
