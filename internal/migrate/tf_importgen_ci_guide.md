...
- **Terraform/OpenTofu**: Version ≥ 1.7 (required for removed blocks, optional)
...
2. **Command Execution Flow**:
   ...
   - Generates removed blocks (`removals.tf`)
...
3. **Generated Files**:
   ...
   - `removals.tf`: Removed blocks for existing NRQL drop rules
...
2. **Update CI Configuration**:
   ...
   - Integrate content from `removals.tf` into your main configuration
3. **Execute Migration**: Run `terraform plan` and `terraform apply` in your CI environment
...
### removals.tf
Contains removed blocks for existing NRQL drop rules:
```hcl
removed {
  from = newrelic_nrql_drop_rule.rule_name
...
### Option 1: Using Removed Blocks (Recommended)

**Requirements**: Terraform/OpenTofu ≥ 1.7

1. Copy all generated files to CI workspace
2. Comment out existing NRQL drop rule resources
3. Integrate `removals.tf` content into main configuration
4. Execute `terraform plan` and `terraform apply`

### Option 2: Manual State Removal

**Requirements**: Terraform/OpenTofu ≥ 1.5

1. Copy generated files (excluding `removals.tf`)
2. Manually remove NRQL drop rules from state:
   ```bash
